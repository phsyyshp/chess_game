...
class PgnParser:
    ...
    def _remove_double_empty_lines(self):
        empty_lines_mask = (self.pgn_lines == "\n")
        empty_lines_mask_left_shifted = np.append(empty_lines_mask[1:], False)
        double_empty_lines_mask = empty_lines_mask_left_shifted * empty_lines_mask
        pgn_lines_no_double_blank_lines= pgn_lines[0 == double_empty_lines_mask] 
        self.pgn_lines = pgn_lines_no_double_blank_lines

    def parse(self):
        self._remove_double_empty_lines()
        self._add_blank_lines_if_missing()
        self._add_blank_lines_if_missing_ab()
        self._replace_game_seperator()
        self._vec_tidy_up()
...
vec_does_contain_bracket = np.vectorize(is_contain_bracket)
    ...
    def replace_game_seperator(arrayed_pgn):
        empty_lines_mask = arrayed_pgn == "\n"
        arrayed_pgn[empty_lines_mask] = "SEPERATOR"
        return arrayed_pgn
...
class PgnToSQL:
    ...
    def pgn_to_sql(pgn_file):
        ...
        with open(pgn_file, "r", encoding="latin-1") as f:
            pgn_lines_all = f.readlines()
        print(len(pgn_lines_all))
        for i in range(500):
            # partition listed pgn into 10 parts and create pgn_lines for i th partition
            pgn_lines = pgn_lines_all[
                i * len(pgn_lines_all) // 500 : (i + 1) * len(pgn_lines_all) // 500
            ]

            arrayed_pgn = np.array(pgn_lines, dtype=str)
