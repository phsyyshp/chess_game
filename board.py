...
from utilities import piece_id_to_color, does_rank_change
...
class board:
    ...
    def set_board_to_initial_configuration(self):
        ...
        self.board_matrix[0][:] = np.array(
            [
                white_piece_id_to_name["rook"],
                white_piece_id_to_name["knight"],
                white_piece_id_to_name["bishop"],
                white_piece_id_to_name["queen"],
                white_piece_id_to_name["king"],
                white_piece_id_to_name["bishop"],
                white_piece_id_to_name["knight"],
                white_piece_id_to_name["rook"],
            ]
    ...
    def is_destination_occupied_by_same_color(
        self, piece_object, new_position_row_column
    ):
        piece_id_at_new_position = self.get_piece_id_from_position(
            new_position_row_column
        )
        return (
            piece_object.color() == piece_id_to_color(piece_id_at_new_position)
            if not self.is_square_empty(new_position_row_column)
            else False
        )
    ...
    def is_pawn_path_clear(self, piece_object, new_position_row_column):
        if piece_object.is_capture_attempt(new_position_row_column):
            return not self.is_destination_occupied_by_same_color(
                piece_object, new_position_row_column
            ) and not self.is_square_empty(new_position_row_column)
        else:
    ...
    def is_castling_attempt(self, piece_object, new_position_row_column):
        if piece_object.type() == "king":
            return (
                abs(piece_object.position_row_column[1] - new_position_row_column[0])
            ) == 2 and not (
                does_rank_change(
                    piece_object.position_row_column, new_position_row_column
                )
            )
    ...
    def is_move_legal(self, old_position_row_column, new_position_row_column):
        ...
        if self.is_square_empty(old_position_row_column):
            ...
            return self.is_path_clear(
                piece_object, new_position_row_column
            ) and self.is_safe(old_position_row_column, new_position_row_column)
    ...
    def move_general_input(self, movement_str):
        ...
        match self.type_of_movement(movement_str):
            ...
            case "regular":
                (
                    old_position_row_column,
                    new_position_row_column,
                ) = self.movement_str_to_old_new_position_row_column(movement_str)
                self.move(old_position_row_column, new_position_row_column)

    def move(self, old_position_row_column, new_position_row_column):
        self.board_matrix[new_position_row_column[0]][
            new_position_row_column[1]
        ] = self.get_piece_id_from_position(old_position_row_column)
